    ...
    const handleFollowToggle = async () => {
        ...
        try {
            if (followStatus === "not_following") {
                ...
                if (data.success) {
                    setFollowStatus(data.message.toLowerCase().includes("request") ? "requested" : "following")
                }
        ...
        <div className="min-h-screen bg-[#FBFBFD] dark:bg-[#000000] text-[#1D1D1F] dark:text-[#F5F5F7] font-sans">
            ...
            <main className="max-w-4xl mx-auto pt-12 pb-24 px-5">
                ...
                <section className="flex flex-col md:flex-row gap-12 items-center md:items-start mb-16">
                    {/* AVATAR SECTION - Apple Squircle with Your Gradient/Shadow */}
                    <motion.div
                    ...
                    >
                        <div className="flex flex-col md:flex-row md:items-center gap-6">
                            ...
                            <div className="flex gap-3 justify-center">
                                <button
                                    ...
                                    className="px-8 py-2.5 bg-blue-600 text-white rounded-xl text-[10px] font-black uppercase tracking-[0.2em] shadow-lg shadow-blue-500/20 active:scale-95 transition-transform"
                                >
                                    {isProcessing ? "SYNCING..." : followStatus.replace('_', ' ')}
                                </button>
                        ...
                        <div className="max-w-md mx-auto md:mx-0 bg-white dark:bg-white/[0.03] p-6 rounded-[32px] border border-zinc-100 dark:border-white/5 shadow-sm text-left">
                            ...
                            {profile.website && (
                                <a href={profile.website} target="_blank" className="inline-flex items-center gap-2 text-blue-600 dark:text-blue-400 text-[11px] font-black uppercase tracking-widest mt-4 hover:opacity-70 transition-opacity">
                ...
                <div className="space-y-8">
                    ...
                    {profile.isPrivate && followStatus !== "following" ? (
                        ...
                        <div className="grid grid-cols-3 gap-1 md:gap-3.5">
                            <div className="col-span-3 py-32 flex flex-col items-center text-center text-zinc-400 space-y-3">
                                <Camera size={36} strokeWidth={1} />
                                <h3 className="text-lg font-semibold tracking-tight">No content yet.</h3>
                            </div>
                        </div>
                    )}
                </div>
            ...
            <AnimatePresence>
                {modalConfig?.isOpen && profile?.user?._id && (
                    <UsersListModal
...
function UsersListModal({ userId, type, onClose }: { userId: string, type: 'followers' | 'following', onClose: () => void }) {
        ...
        const fetchUsers = async () => {
            try {
                ...
                const [listRes, followingRes] = await Promise.all([
                    fetch(`${API_BASE}/follow/${userId}/${type}`, { headers: { Authorization: `Bearer ${token}` } }),
                    fetch(`${API_BASE}/follow/${userId}/following`, { headers: { Authorization: `Bearer ${token}` } })
                ])
                ...
                const initializedUsers = listData.data?.map((item: any) => {
                    ...
                    return { ...item, isFollowing: followingIds.has(u._id) }
                }) || []
    ...
    const handleFollowToggle = async (e: React.MouseEvent, targetUser: any) => {
        ...
        const current = users.find(item => (type === "followers" ? item.follower : item.following)._id === targetId);
        const isCurrentlyFollowing = current?.isFollowing;

        setUsers(prev => prev.map(item => {
            const u = type === "followers" ? item.follower : item.following;
            return u._id === targetId ? { ...item, isFollowing: !item.isFollowing } : item;
        }));

        try {
            const endpoint = isCurrentlyFollowing ? `${API_BASE}/follow/${targetId}/unfollow` : `${API_BASE}/follow/${targetId}/follow`;
            const method = isCurrentlyFollowing ? "DELETE" : "POST";
            await fetch(endpoint, { method, headers: { Authorization: `Bearer ${myToken}` } });
        } catch (err) {
            setUsers(prev => prev.map(item => {
                const u = type === "followers" ? item.follower : item.following;
                return u._id === targetId ? { ...item, isFollowing: !item.isFollowing } : item;
    ...
    const filteredUsers = users.filter(item => {
        const u = type === 'followers' ? item.follower : item.following;
        return u?.username?.toLowerCase().includes(searchQuery.toLowerCase()) || u?.name?.toLowerCase().includes(searchQuery.toLowerCase());
    });
            ...
            >
                <div className="p-6 border-b border-zinc-200 dark:border-white/10 space-y-4">
                    <div className="flex items-center justify-between">
                        <h3 className="text-xl font-bold uppercase tracking-tight italic leading-none">{type}</h3>
                    ...
                    <div className="relative flex items-center">
                        ...
                        <input
                            ...
                            className="w-full bg-zinc-100 dark:bg-black/20 rounded-full py-2.5 pl-10 pr-5 text-base outline-none transition-all placeholder:text-zinc-500"
                        />
                    </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-4 custom-scrollbar min-h-[300px]">
                    {loading ? (
                        ...
                        <div className="py-20 text-center text-zinc-400"><Users size={32} className="mx-auto mb-4" /><p className="text-sm font-medium tracking-tight">No results.</p></div>
                    ) : (
                        <div className="space-y-3">
                            {filteredUsers.map((item: any) => {
                                ...
                                if (!u) return null;
                                return (
                                    <div key={u._id} onClick={() => { router.push(`/profile/${u.username}`); onClose(); }} className="flex items-center justify-between p-2 rounded-xl hover:bg-zinc-50 dark:hover:bg-black/20 transition-all cursor-pointer group">
                                        <div className="flex items-center gap-3.5">
                                            <div className="w-12 h-12 rounded-full overflow-hidden border border-zinc-200 dark:border-white/10">
                                        ...
                                        <button onClick={(e) => handleFollowToggle(e, u)} className={`px-5 py-2 rounded-full text-sm font-semibold transition-all active:scale-[0.98] ${item.isFollowing ? "bg-zinc-100 dark:bg-zinc-800 text-blue-600 dark:text-blue-500" : "bg-blue-600 text-white"}`}>{item.isFollowing ? "Remove" : "Follow"}</button>
                                    </div>
                                )
                            })}
                        </div>
...
function TabButton({ icon, label, active, onClick }: any) {
    return (
        <button
            onClick={onClick}
            className={`relative flex items-center justify-center gap-3 flex-1 py-4 px-2 transition-all ${active ? "text-blue-500" : "text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-200"}`}
        >
            <span className="relative z-10">{icon}</span>
            <span className="relative z-10 text-[10px] font-black uppercase tracking-[0.2em] italic hidden md:block">
                {label}
            </span>
            {active && (
                <motion.div
...
function LoadingSkeleton() {
    return (
        <div className="min-h-screen bg-[#FBFBFD] dark:bg-[#000000] flex flex-col items-center justify-center gap-8">
            ...
            />
            <div className="text-[10px] font-black text-blue-500 tracking-[0.5em] uppercase animate-pulse italic">
                Synchronizing...
            </div>
        </div>
    )
